...
#include <QPainter>
#include <iostream>
S11Param::S11Param(QWidget* parent)
    : QWidget(parent),
...
void S11Param::Load()
{
    ...
    max = 0;
    for (int i = 0; i < s.S[0][0].size(); i++)
    {
        z.append(s.S[0][0][i].real());
        if (z[i] > max)
        {
            max = z[i];
        }
...
QPointF S11Param::compute_real(float t)
{
    float cos_t = cos(t);
    float sin_t = sin(t);
...
QPointF S11Param::compute_imaginary(float t)
{
    float cos_t = cos(t);
    float sin_t = sin(t);
    float x = 1 + (1 / r) * cos_t;
...
void S11Param::drawStaticObjects(QPainter& painter)
{
    ...
    painter.setPen(Qt::blue);
    double m = 0;
    for (S11Param::r = -10; S11Param::r <= 10; S11Param::r += 0) {
        if (r == -10)
        {
            m = -8;
        }
    ...
    double k = 0.125;
    for (S11Param::r = 0; S11Param::r < 10; S11Param::r += 0) {
        if (r == 0.25)
        {
            r = 0.2;
        }
    ...
    k = 0.25;
    for (S11Param::r = 0.25; S11Param::r < 10; S11Param::r += 0) {
        if (r == 0.25)
        {
            r = 0.2;
        }
...
void S11Param::paintEvent(QPaintEvent* event)
{
    scaleFactor = qMin(this->width(), this->height()) / 450.0f;
    TouchstoneFile t;
    spar_t s;
    ...
    if (highlightedPoint >= 0 && highlightedPoint < x.size()) {
        ...
        complex_t s01 = s.S[0][0][highlightedPoint];
...
void S11Param::highlightPoint(int index)
{
